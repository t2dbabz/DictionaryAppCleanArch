package com.example.dictionary.feature_dictionary.presentation.components
...
fun WordInfoItem(wordInfo: WordInfo, modifier: Modifier = Modifier) {
    ...
    Column(modifier = modifier) {
        ...
        wordInfo.meaning.forEach { meaning ->
            ...
            meaning.definitions.forEachIndexed{index, definitions->
                Text(text = "${index + 1}. ${definitions.definition}",)
                Spacer(modifier = Modifier.height(8.dp))
                definitions.example?.let { example ->
